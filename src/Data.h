...
class ETData {
   ...
   public:
    ...
    // to read data from one file, one instrument
    void load(const string filename, int skip = 2);
   ...
   private:
    // Singleton
    static ETData ETinstance;

   public:
    static ETData& get_instance() { return ETinstance; }
};

template <class... Args>
void load_et(Args&&... args)
{
    ETData::get_instance().load(args...);
}
