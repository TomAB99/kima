...
void RV_binaries_model::setPriors()  // BUG: should be done by only one thread!
{
    auto data = get_data();
    ...
    if (known_object) { // KO mode!
        ...
        for (int i = 0; i < n_known_object; i++){
            ...
            if (!eclipsing)
                KO_cosincprior[i] = make_prior<Uniform>(0,1);
                cout << 'yes1';
        }
    }
...
void RV_binaries_model::from_prior(RNG& rng)
{
    // preliminaries
    setPriors();
    save_setup();
    ...
    if (known_object) { // KO mode!
        ...
        if (!eclipsing)
            KO_cosinc.resize(n_known_object);
        ...
        for (int i=0; i<n_known_object; i++){
            ...
            if (eclipsing)
                KO_cosinc[i] = KO_cosincprior[i]->generate(rng);
        }
    }
 ...
 * 
*/
void RV_binaries_model::calculate_C()
{
    // Get the data
    auto data = get_data();
 ...
 * 
*/
void RV_binaries_model::calculate_mu()
{
    auto data = get_data();
...
void RV_binaries_model::remove_known_object()
{
    auto data = get_data();
    auto t = data.get_t();
    double f, v, delta_v, ti, Tp, w_t, P_anom, cosinc;
    ...
    for(int j=0; j<n_known_object; j++)
    {
        for(size_t i=0; i<t.size(); i++)
        {
            ...
            delta_v = postKep::post_Newtonian(KO_K[j],f,KO_e[j],w_t,P_anom,cosinc,star_mass,binary_mass,star_radius);
            v += delta_v;
            mu[i] -= v;
        }
    }
...
void RV_binaries_model::remove_known_object_secondary()
{
    auto data = get_data();
    auto t = data.get_t();
    double f, v, delta_v, ti, Tp, w_t, P_anom, K2, cosinc;
    // cout << "in remove_known_obj: " << KO_P[1] << endl;
    for(int j=0; j<n_known_object; j++)
    {
        for(size_t i=0; i<t.size(); i++)
        {
            ...
            delta_v = postKep::post_Newtonian(K2,f,KO_e[j],w_t,P_anom,cosinc,binary_mass,star_mass,star_radius);
            v += delta_v;
            mu_2[i] -= v;
        }
    }
...
int RV_binaries_model::is_stable() const
{
    // Get the components
...
double RV_binaries_model::perturb(RNG& rng)
{
    #if TIMING
    auto begin = std::chrono::high_resolution_clock::now();  // start timing
    #endif
        ...
        else if(rng.rand() <= 0.5) // perturb jitter(s) + known_object
        {
            ...
            if (known_object)
            {
                ...
                for (int i=0; i<n_known_object; i++){
                    ...
                    if (!eclipsing)
                        KO_cosincprior[i]->perturb(KO_cosinc[i], rng);
                }
...
*/
double RV_binaries_model::log_likelihood() const
{
    auto data = get_data();
    int N = data.N();
    auto y = data.get_y();
...
void RV_binaries_model::print(std::ostream& out) const
{
    // output precision
    ...
    if(known_object){ // KO mode!
        ...
        if (!eclipsing)
            for (auto cosinc: KO_cosinc) out << cosinc << "\t";
    }
...
string RV_binaries_model::description() const
{
    ...
    if(known_object) { // KO mode!
        ...
        {
            for(int i=0; i<n_known_object; i++) 
                desc += "KO_cosinc" + std::to_string(i) + sep;
        }
    }
...
void RV_binaries_model::save_setup() {
    ...
    if (known_object) {
        ...
        for(int i=0; i<n_known_object; i++){
            ...
            if (!eclipsing)
                fout << "cosincprior_" << i << ": " << *KO_cosincprior[i] << endl;
        }
    }
